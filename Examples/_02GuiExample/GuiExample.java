package Examples._02GuiExample;
...
class CACell extends AgentSQ2Dunstackable<StemCellCA> {
    ...
    void Init(int divs, boolean stem){
        ...
        if(G().vis!=null) {
            if (stem) {
                G().vis.SetPix(Xsq(), Ysq(), StemCellCA.BLUE);
            }
            else{
                G().vis.SetPix(Xsq(), Ysq(), RGB((double) (((divs + G().maxDivs) * 1.0f) / (G().maxDivs * 2)), (double) 0, (double) 0));
            }
        }
    }
    void Die(){
...
class StemCellCA extends AgentGrid2D<CACell> {
    static final int WHITE =RGB(1,1,1),BLACK=RGB(0,0,0),BLUE=RGB(0,0,1);
    ...
    public static void main(String[] args){
        ...
        win.AddCol(0, new GuiDoubleField("divProb",1.0/24,0,1).SetColor(WHITE, BLACK));
        win.AddCol(0, new GuiDoubleField("deathProb",1.0/1000,0,1).SetColor(WHITE, BLACK));
        ...
        win.SetColor(BLACK);
        win.AddCol(0, new GuiButton("Run",true,(clickEvent)->{
            ...
            StemCellCA runGrid=new StemCellCA(win.GetInt("worldDims"),win.GetInt("worldDims"),win.GetDouble("divProb"),
                    win.GetDouble("deathProb"),win.GetDouble("stemDivProb"),win.GetInt("maxDivs"),vis);
