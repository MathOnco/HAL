...
public class ContactInhibitionInteractive extends AgentGrid2D<Cell> implements TreatableTumor,Serializable{
    ...
    public void Draw(GuiGridVis vis,GuiGridVis alphaVis,boolean[] switchVals) {
        for (int i = 0; i < vis.length; i++) {
            ...
            alphaVis.SetPix(i, SetAlpha(HeatMapRGB(drug.Get(i)*8.0/3.0), Utils.Bound(drug.Get(i)*8.0,0,0.5)));
        }
    }
...
//    }
    ...
    public static void main(String[] args) {
        ...
        ClinicianSim ccl=new ClinicianSim(jm,600,10,5,1,30,25,10,100);
        ccl.RunGui();
        ccl.RunModel();
    }
...
class Cell extends AgentSQ2Dunstackable<ContactInhibitionInteractive> {
    ...
    public void CellStep(Random rn) {
        ...
        if (rn.nextDouble() < G().deathProb + (isRes ? 0 : G().drug.Get(Isq()) * G().drugDeath)) {

            Dispose();
        }
        //Chance of Division, depends on resistance
        else if (rn.nextDouble() < (isRes ? G().divProbRes : G().divProb)) {
