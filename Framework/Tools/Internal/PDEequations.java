...
public class PDEequations {
    ...
    public static void Diffusion2inhomogeneous(int x,int y,final double[] inGrid, final double[] outGrid,final double[] diffRates, final int xDim, final int yDim, final boolean boundaryCond, final double boundaryValue, final boolean wrapX,final boolean wrapY) {
        ...
        currRate = GetDisplacedX(x + 1, y, diffRates, xDim, yDim, x - 1, false, boundaryValue, wrapX);
        valSum += GetDisplacedX(x + 1, y, inGrid, xDim, yDim, x - 1, boundaryCond, boundaryValue, wrapX) * currRate;
        rateSum += currRate;
        currRate = GetDisplacedX(x - 1, y, diffRates, xDim, yDim, x + 1, false, boundaryValue, wrapX);
        valSum += GetDisplacedX(x - 1, y, inGrid, xDim, yDim, x + 1, boundaryCond, boundaryValue, wrapX) * currRate;
        rateSum += currRate;
        currRate = GetDisplacedY(x, y+1, diffRates, xDim, yDim, y - 1, false, boundaryValue, wrapX);
        valSum += GetDisplacedY(x, y+1, inGrid, xDim, yDim, y - 1, boundaryCond, boundaryValue, wrapX) * currRate;
        rateSum += currRate;
        currRate = GetDisplacedY(x, y-1, diffRates, xDim, yDim, y + 1, false, boundaryValue, wrapX);
        valSum += GetDisplacedY(x, y-1, inGrid, xDim, yDim, y + 1, boundaryCond, boundaryValue, wrapX) * currRate;
        rateSum += currRate;
    ...
    public static void Diffusion2(int x,int y,final double[] inGrid, final double[] outGrid, final int xDim, final int yDim, final double diffRate, final boolean boundaryCond, final double boundaryValue, final boolean wrapX,final boolean wrapY) {
        ...
        double valSum=GetDisplacedX(x+1,y,inGrid,xDim,yDim,x,boundaryCond,boundaryValue,wrapX);
        valSum+=GetDisplacedX(x-1,y,inGrid,xDim,yDim,x,boundaryCond,boundaryValue,wrapX);
        valSum+=GetDisplacedY(x,y+1,inGrid,xDim,yDim,y,boundaryCond,boundaryValue,wrapY);
        valSum+=GetDisplacedY(x,y-1,inGrid,xDim,yDim,y,boundaryCond,boundaryValue,wrapY);
        int i=x*yDim+y;
    ...
    public static void Advection2ndOrder(int x,int y,final double[]inGrid,final double[]outGrid,int xDim,int yDim,double xVel,double yVel,boolean boundaryCond,double boundaryValue) {
        ...
        if(xVel>0){
            xFlux=(xVel/2)*(3*prev
                    -4*GetDisplacedX(x-1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)
                    +GetDisplacedX(x-2,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
        }
        if(xVel<0){
            xFlux=(xVel/2)*(-3*prev
                    +4*GetDisplacedX(x+1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)
        ...
        if(yVel>0){
            yFlux=(yVel/2)*(3*prev
                    -4*GetDisplacedY(x,y-1,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)
                    +GetDisplacedY(x,y-2,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
    ...
    public static void Advection2ndOrderCorrection(int x,int y,final double[]intGrid,final double[]outGrid,int xDim,int yDim,double xVel,double yVel,boolean boundaryCond,double boundaryValue) {
        ...
        if(xVel>0){
            xFlux=(xVel/2)*(3*prev
                    -4*GetDisplacedX(x-1,y,intGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)
                    +GetDisplacedX(x-2,y,intGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
        }
        if(xVel<0){
            xFlux=(xVel/2)*(-3*prev
                    +4*GetDisplacedX(x+1,y,intGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)
        ...
        if(yVel>0){
            yFlux=(yVel/2)*(3*prev
                    -4*GetDisplacedY(x,y-1,intGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)
                    +GetDisplacedY(x,y-2,intGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
    ...
    public static void Advection3rdOrder(int x,int y,final double[]inGrid,final double[]outGrid,int xDim,int yDim,double xVel,double yVel,boolean boundaryCond,double boundaryValue) {
        ...
        double prev=inGrid[i];
        if(xVel>0){
            xFlux=(xVel/6)*(2*GetDisplacedX(x+1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)+3*prev
                    -6*GetDisplacedX(x-1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)
                    +GetDisplacedX(x-2,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
        }
        if(xVel<0){
            xFlux=(xVel/6)*(-2*GetDisplacedX(x-1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)-3*prev
        ...
        if(yVel>0){
            yFlux=(yVel/2)*(2*GetDisplacedX(x,y+1,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)+3*prev
                    -6*GetDisplacedY(x,y-1,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)
                    +GetDisplacedY(x,y-2,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
    ...
    public static void Advection2ndOrderLW(int x, int y, final double[] inGrid, final double[] outGrid, int xDim, int yDim, double xVel, double yVel, boolean boundaryCond, double boundaryValue){
        ...
        double yyFlux=0;
        double prev=inGrid[i];
        xFlux=(xVel/2)*(GetDisplacedX(x+1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)-GetDisplacedX(x-1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
        yFlux=(yVel/2)*(GetDisplacedY(x,y+1,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)-GetDisplacedY(x,y-1,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
        //xyFlux=-(xVel*yVel/4)*(GetDisplaced(x+1,y+1,inGrid,xDim,yDim,-1,-1,boundaryCond,boundaryValue,true,true)-GetDisplaced(x-1,y+1,inGrid,xDim,yDim,-1,-1,boundaryCond,boundaryValue,true,true)-GetDisplaced(x+1,y-1,inGrid,xDim,yDim,-1,-1,boundaryCond,boundaryValue,true,true)+GetDisplaced(x-1,y-1,inGrid,xDim,yDim,-1,-1,boundaryCond,boundaryValue,true,true));
        xxFlux=-(xVel*xVel/2)*(GetDisplacedX(x+1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)-2*prev+GetDisplacedX(x-1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
        yyFlux=-(yVel*yVel/2)*(GetDisplacedY(x,y+1,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)-2*prev+GetDisplacedY(x,y-1,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
    ...
    public static void Advection1stOrder(int x, int y, final double[] inGrid, final double[] outGrid, int xDim, int yDim, double xVel, double yVel, boolean boundaryCond, double boundaryValue){
        ...
        double prev=inGrid[i];
        if(xVel>0){
            xFlux=xVel*(prev-GetDisplacedX(x-1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
        }
        if(xVel<0){
            xFlux=xVel*(GetDisplacedX(x+1,y,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)-prev);
        }
        if(yVel>0){
            yFlux=yVel*(prev-GetDisplacedY(x,y-1,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true));
        }
        if(yVel<0){
            yFlux=yVel*(GetDisplacedY(x,y+1,inGrid,xDim,yDim,-1,boundaryCond,boundaryValue,true)-prev);
        }
        outGrid[i]=inGrid[i]-xFlux-yFlux;
    }
